...
const AcademicConfig = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success(`Department ${editMode ? 'updated' : 'added'} successfully`);
            ...
            console.error(`Error ${editMode ? 'updating' : 'adding'} department:`, err);
    ...
    const handleDeleteBatch = async (batch) => {
        if (window.confirm(`Are you sure you want to delete batch ${batch}? This will delete all section count data for this batch.`)) {
            try {
                setLoading(true);
                await deleteBatch({ batch });
                toast.success(`Batch ${batch} deleted successfully`);
                await loadBatches();
                await loadSectionCounts(); // Reload section counts as some will be deleted
            } catch (err) {
                    ...
                    <div className="table-responsive" style={{ padding: "0 1.25rem" }}>
                        <Table style={{ marginBottom: "1.25rem" }} hover>
                            <thead style={{ background: "rgba(194, 137, 248, 0.08)" }}>
                                <tr>
                                    <th style={{ padding: "1rem", fontWeight: 600, color: "rgb(154, 77, 226)" }}>
                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                            <Icon path={mdiSchool} size={0.8} color="rgb(194, 137, 248)" style={{ marginRight: '8px' }} />
                                            Department
                                        </div>
                                    </th>
                                    <th style={{ padding: "1rem", fontWeight: 600, color: "rgb(154, 77, 226)", textAlign: "center" }}>
                                    ...
                                    <th style={{ padding: "1rem", fontWeight: 600, color: "rgb(154, 77, 226)", textAlign: "center" }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <Icon path={mdiViewGridPlus} size={0.8} color="rgb(194, 137, 248)" style={{ marginRight: '8px' }} />
                                            Subsections per Section
                                        </div>
                                    </th>
                                    <th style={{ padding: "1rem", fontWeight: 600, color: "rgb(154, 77, 226)" }}>
                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                            <Icon path={mdiCog} size={0.8} color="rgb(194, 137, 248)" style={{ marginRight: '8px' }} />
                                            Actions
                                        </div>
                                    </th>
                                </tr>
                                ...
                                {loading ? (
                                    ...
                                    departments.map((dept, index) => (
                                        <tr key={dept.department}>
                                            ...
                                            <td style={{ padding: "1rem", textAlign: "center" }}>{dept.section_count}</td>
                                            <td style={{ padding: "1rem", textAlign: "center" }}>{dept.subsection_count_per_section}</td>
            ...
            >
                <Modal.Header style={{
                    ...
                    <Button
                        ...
                        style={{ color: "white", textDecoration: "none", padding: 0, fontSize: "1.5rem" }}
                    >
                        &times;
                    </Button>
                </Modal.Header>
                    ...
                    <Form>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Text className="text-muted">
                                Adding a new batch will automatically create section counts for all departments using the default values.
                            </Form.Text>
                        </Form.Group>
                    </Form>
                </Modal.Body>
                    ...
                    <Form>
                        <div style={{ marginBottom: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                                ...
                                <span style={{ marginLeft: '8px' }}>{editSectionCount.batch}</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                <Icon path={mdiSchool} size={0.8} color="rgb(194, 137, 248)" style={{ marginRight: '8px' }} />
                                <strong style={{ color: 'rgb(154, 77, 226)' }}>Department:</strong>
                                <span style={{ marginLeft: '8px' }}>{editSectionCount.department}</span>
                            </div>
                        </div>
                ...
                <Card.Body style={{ padding: "2rem" }}>
                    <div style={{ borderBottom: "3px solid rgb(194, 137, 248)", paddingBottom: "16px", marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <h4 className="card-title" style={{
                            ...
                            <span style={{ marginRight: "12px" }}>
                                <Icon path={mdiViewGridPlus} size={1.2} color="rgb(194, 137, 248)" />
                            </span>
                    ...
                    <div className="table-responsive" style={{ padding: "0 1.25rem" }}>
                        <Table style={{ marginBottom: "1.25rem" }} hover>
                            ...
                            <tbody>
                                {loading ? (
                                ...
                                ) : !Array.isArray(sectionCounts) || sectionCounts.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" className="text-center py-4">
                                    ...
                                    sectionCounts.map((sectionCount, index) => (
                                        <tr key={`${sectionCount.batch}-${sectionCount.department}`}>
                                            <td style={{ padding: "1rem" }}>{sectionCount.department}</td>
                                            <td style={{ padding: "1rem" }}>{sectionCount.batch}</td>
                                            <td style={{ padding: "1rem", textAlign: "center" }}>{sectionCount.section_count}</td>
                                            <td style={{ padding: "1rem", textAlign: "center" }}>{sectionCount.subsection_count_per_section}</td>
                                            <td style={{ padding: "1rem" }}>
                                                <Button
                                                    variant="outline-primary"
                                                    size="sm"
